...
class Tank(ABC):
    ...
    def set_tank_profile(self, segments: dict):
        pass
...
class Profile:
    ...
    def get_profile_points(self) -> list[np.ndarray]:
        ...
        for segment in self.segments:
            points.append(
                np.array([segment.r_start, segment.y_start])
            )
        points.append(
            np.array([self.segments[-1].r_end, self.segments[-1].y_end])
        )
    ...
    def get_profile_normals(self) -> list[np.ndarray]:
        ...
        for segment in self.segments:
            normals.append(
                segment.get_normal(segment.y_start)
            )
        normals.append(
            self.segments[-1].get_normal(self.segments[-1].y_end)
        )
        return normals
...
# profile1 = Profile(segments=[ellipse1, line1, ellipse2])
...
# tank_profile.insert_interface(tol=0.02, x_wall=3e-3)
# y_interface = tank_profile.y_interface
# tank_profile.get_curve_groups()

# tank_profile.split_profile(y_interface - 0.1, tol=0.005)
# tank_profile.split_profile(y_interface + 0.1, tol=0.005)


def plot_profile(ax: Axes) -> None:
    """
    Plot the profile on the given axis.
    """
    for segment in tank_profile.segments:
        y = np.linspace(segment.y_start, segment.y_end, 1000)
        r = np.array([segment.get_radius(yi) for yi in y])
        if len(y) == 0:
            y = np.array([segment.y_start, segment.y_end])
